...
def render_ui(data: Data):
    option_string_short = "%.9i %.23j %.2t %.8M"
    option_string_medium = "%.9i %.23j %.2t %.8M %.9m %.4C"
    option_string_long = "%.9i %.23j %.2t %.8M %.6D %.8Q %.12l %.9m %.4C %.16S %r"
    
    data.stdscr.erase()
    rows, cols = data.stdscr.getmaxyx()

    if cols <= 20 or rows < 1:
        data.stdscr.addstr(0, 1, "TO SMALL")
        return
    
    elif cols <= 46 or rows <= 4 + len(get_job_ids()) + len(data.shell_scripts):
        x = (cols // 2) - 10
        y = (rows // 2) - 1
        data.stdscr.addstr(y, x, "Window is to small!")
        return

    option_string = option_string_long
    if cols <= 61:
        option_string = option_string_short
    elif cols <= 113:
        option_string = option_string_medium


    data.stdscr.addstr(0, 0, os.popen(f'squeue -u $USER -o "{option_string}"').read())
    info_string = 'Switch lists with right/left arrow, sbatch with s, scancel with c, quit with q'

    if cols > (len(info_string) + 4):
        data.stdscr.addstr(rows -1, 2, info_string)
